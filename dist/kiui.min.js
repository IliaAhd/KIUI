(()=>{async function l(i="./data.json"){try{let e=await fetch(i);if(!e.ok)throw new Error("Failed to fetch data");return(await e.json()).data}catch(e){return console.error("Error loading Data:",e),null}}function d(i,e){let t=i.classList.contains("open");i.classList.contains("translate-x-full")?(i.classList.toggle("translate-x-0"),i.classList.toggle("translate-x-full")):i.classList.toggle("hidden");let[s,a]=e.children;t?(s.classList.remove("opacity-0"),s.classList.add("opacity-100"),a.classList.remove("opacity-100"),a.classList.add("opacity-0"),i.classList.remove("open")):(s.classList.remove("opacity-100"),s.classList.add("opacity-0"),a.classList.remove("opacity-0"),a.classList.add("opacity-100"),i.classList.add("open"))}var o=class extends HTMLElement{async connectedCallback(){let e=await l();if(!e)return;let t=e.menu.items.map(s=>`<li class="menu-item"><a class="transition-colors">${s}</a></li>`).join("");this.render(e.menu.title,t),this.setLinks(e.menu.links),this.setupMobileToggle()}render(e,t){this.innerHTML=`
    ...
    `}setLinks(e){this.querySelectorAll(".menu-item a").forEach((s,a)=>{s.setAttribute("href",e[a]||"#")})}setupMobileToggle(){let e=this.querySelector("#hamburger-btn"),t=this.querySelector("#mobile-menu"),s=e;e&&t&&s&&(s.children[0].classList.add("opacity-100"),s.children[1].classList.add("opacity-0"),e.addEventListener("click",()=>d(t,s)))}},r=class extends HTMLElement{async connectedCallback(){let e=await l();if(!e)return;let t=e.menu.items.map(s=>`<li class="menu-item"><a class="transition-colors">${s}</a></li>`).join("");this.render(e.menu.title,t),this.setLinks(e.menu.links),this.setupMobileToggle()}render(e,t){this.innerHTML=`
    ...
    `}setLinks(e){this.querySelectorAll(".menu-item a").forEach((s,a)=>{s.setAttribute("href",e[a]||"#")})}setupMobileToggle(){let e=this.querySelector("#hamburger-btn"),t=this.querySelector("#mobile-menu"),s=e;e&&t&&s&&(s.children[0].classList.add("opacity-100"),s.children[1].classList.add("opacity-0"),e.addEventListener("click",()=>d(t,s)))}};customElements.define("header-type-1",o);customElements.define("header-type-2",r);var n=class extends HTMLElement{async connectedCallback(){let e=await l();e&&this.render(e.footer)}render(e){this.innerHTML=`
    ...
    `}},c=class extends HTMLElement{async connectedCallback(){let e=await l();e&&this.render(e.footer)}render(e){this.innerHTML=`
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-10 text-center md:text-left border-t border-gray-700 pt-10">
          ${Object.values(e.footerItems).map(t=>`
            <div>
              ...
              <ul class="space-y-2 text-sm">
                ${t.items.map((s,a)=>`<li><a href="${t.links[a]}" class="hover:underline">${s}</a></li>`).join("")}
              </ul>
            </div>
          `).join("")}
        </div>
    ...
    `}};customElements.define("footer-type-1",n);customElements.define("footer-type-2",c);})();
