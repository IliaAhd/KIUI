    ...
    this.innerHTML = `
      ...
      <div class="w-87 bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-md hover:shadow-xl hover:-translate-y-2 transition duration-300">
        ...
        <div class="h-42 p-5 flex flex-col justify-between space-y-3">
          ${
            title
              ? `<h2 class="text-lg sm:text-xl font-semibold text-gray-800 dark:text-white">${title}</h2>`
              : ""
          }
          ${
            description
          ...
          ${
            price || buttonLink
              ? `<div class="flex items-center justify-between mt-4">
                  ${
                    price
                      ? `<span class="text-lg font-bold text-slate-600 dark:text-indigo-400">$${price}</span>`
                      : "<div></div>"
                  }
                  ${
                    buttonTitle
                      ? `<a href="${buttonLink}" target="_blank" rel="noopener noreferrer" class="px-4 py-2 bg-slate-600 text-white rounded-xl hover:bg-slate-800 cursor-pointer transition">${buttonTitle}</a>`
                      : ""
                  }
                </div>`
              : ""
